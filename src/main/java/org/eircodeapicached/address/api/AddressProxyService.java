...
public class AddressProxyService {
	...
	public static final String REQ_PARAM_MOCK_MODE_PARAM = "param";
	public static final String REQ_PARAM_MOCK_MODE_DATA = "data";
	...
	private PostcoderAddressClient addressMockClient;

	@Resource(name="paramCheckMockClient")
	private PostcoderAddressClient paramCheckMockClient;
	...
	public ResponseEntity<String> address(
			...
			@RequestParam(required=false) String mockMode) {
		...
		String response;
		if (REQ_PARAM_MOCK_MODE_PARAM.equals(mockMode)) {
			log.debug("Request parameter mockMode=param detected. Using ParamCheckMock client to render response.");
			response = paramCheckMockClient.address(apiKey, country, eircodeOrAddressFrag,
					format, acceptFormat, lines, page, include, exclude,
					addtags, identifier, callback, postcodeonly);

		} else if (REQ_PARAM_MOCK_MODE_DATA.equals(mockMode)) {
			log.debug("Request parameter mockMode=data detected. Using Mock client to render response.");
			response = addressMockClient.address(apiKey, country, eircodeOrAddressFrag,
					format, acceptFormat, lines, page, include, exclude,
					addtags, identifier, callback, postcodeonly);
		} else {
