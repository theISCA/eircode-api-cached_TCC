...
public class AddressProxyServiceTest {
	...
	/**
	 * Comment the line below and uncomment the line below it
	 * to perform tests against the Postcoder web service
	 * itself. For CI to work it should be contain the mockMode
	 * request parameter.
	 */
...
//	private static final String MOCK_DATA_PARAM = "";
	...
	public void testAddressUrlParameterPassing() throws Exception {
		...
		this.mockMvc.perform(get(uri.toString() + "&mockMode=" + AddressProxyService.REQ_PARAM_MOCK_MODE_PARAM))
	...
	public void testAcceptHeaderXML() throws Exception {
		...
		this.mockMvc.perform(get(uri.toString() + "?mockMode=" +
		    AddressProxyService.REQ_PARAM_MOCK_MODE_PARAM).header("Accept", "application/xml"))
			.andExpect(status().isOk())
			.andExpect(header().string("Content-type", "application/xml"))
			.andExpect(content().string(containsString("123")))
			.andExpect(content().string(containsString("ie")))
	...
	public void testValidLocationQueryXML() throws Exception {
		String serviceUrl = "/pcw/PCW45-12345-12345-1234X/address/uk/manor farm barns?format=xml"
				+ MOCK_DATA_PARAM;
